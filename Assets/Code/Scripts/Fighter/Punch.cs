...
public class Punch : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        EventHandler.GotKill += kill_reward;
        EventHandler.EndScenario += reset_punch;
    }
    ...
    public void kill_reward(GameObject killer, GameObject target)
    {
        if (killer == transform.root.gameObject)
        {
            //damage++;
        }
        else if (target == transform.root.gameObject)
        {
            damage = 1;
        }
    }

    public void reset_punch()
    {
        damage = 1;
    }
}
