...
public class PongInstance : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (PongUI.instance.GetActiveCount() < 2)
        {
            ...
            {
                Instantiate(power_ups[Random.Range(0, power_ups.Count)]).transform.position = new Vector3(Random.Range(left_bound, right_bound) * 0.8f, 2, Random.Range(lower_score_bounds, upper_score_bounds) * 0.6f);
